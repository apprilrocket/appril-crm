-- Discovery · result_url persistente (aprobado por Mauricio, 30-jul-2026)
--
-- Objetivo: que el resultado del Discovery tenga un link permanente y seguro
-- (https://discovery.appril.co/?r=<token>) para poder enviarlo por WhatsApp
-- (manual hoy vía inbox en ventana abierta; por template cuando Meta apruebe
-- uno con URL) y para que el usuario pueda reabrirlo.
--
-- Piezas:
--  1. discovery_leads.result_token — token bearer 32-hex no enumerable (mismo
--     patrón que dl_token), DEFAULT en filas nuevas + backfill del histórico.
--  2. RPC discovery_get_result(input jsonb) — anon A PROPÓSITO (funnel público):
--     token → payload de render (raw_answers + frontend_calculations + campos
--     backend). PII mínima: solo el PRIMER nombre; token inválido/inexistente/
--     error → {ok:true, found:false} (anti-enumeración, fail-closed).
--  3. submit_discovery_lead v4 — devuelve result_url en el RETURN y lo sella en
--     el metadata del evento discovery_form_submitted (visible en el timeline
--     del CRM para envío manual por inbox).
--
-- A PROPÓSITO NO se toca el payload del followup WA (queue-sender sin
-- wa_components manda TODOS los valores del payload como parámetros del
-- template: añadir result_url rompería el conteo de los templates actuales).
-- El template WA con URL se cablea cuando exista y esté aprobado en Meta.
-- El link no expira (decisión inicial; revocable borrando/rotando el token).

-- ─── 1) Token por resultado ─────────────────────────────────────────────────
...
UPDATE public.discovery_leads
   SET result_token = replace(gen_random_uuid()::text, '-', '')
 WHERE result_token IS NULL;

COMMENT ON COLUMN public.discovery_leads.result_token IS
  'Token bearer del link permanente al resultado (?r=). No enumerable; tratarlo como secreto del destinatario.';

-- ─── 2) RPC de lectura del resultado ────────────────────────────────────────

CREATE OR REPLACE FUNCTION public.discovery_get_result(input jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_token text := lower(nullif(trim(input->>'token'), ''));
  r record;
BEGIN
  ...
  SELECT id, lead_id, full_name, raw_answers, frontend_calculations,
         score, marketing_segment, agenda_maturity_level, recommended_action, created_at
    INTO r
    FROM public.discovery_leads
   WHERE workspace_id = v_workspace_id AND result_token = v_token
   LIMIT 1;
  ...
  RETURN jsonb_build_object(
    'ok', true, 'found', true,
    'first_name', nullif(split_part(trim(coalesce(r.full_name, '')), ' ', 1), ''),
    'raw_answers', coalesce(r.raw_answers, '{}'::jsonb),
    'frontend_calculations', coalesce(r.frontend_calculations, '{}'::jsonb),
    'lead_score', r.score,
    'marketing_segment', r.marketing_segment,
    'agenda_maturity_level', r.agenda_maturity_level,
    'recommended_action', r.recommended_action,
    'discovery_lead_id', r.id,
    'lead_id', r.lead_id,
...
REVOKE ALL ON FUNCTION public.discovery_get_result(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.discovery_get_result(jsonb) TO anon, authenticated, service_role;

COMMENT ON FUNCTION public.discovery_get_result(jsonb) IS
  'Funnel Discovery público (anon A PROPÓSITO): resuelve result_token → payload de render del resultado. Primer nombre solamente; anti-enumeración.';

-- ─── 3) submit_discovery_lead v4: result_url en RETURN y en el evento ───────
