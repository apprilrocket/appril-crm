-- Cableado del template Meta `diagnostico_es` (APROBADO por Meta el 30-jul-2026)
--
-- 1. message_templates: fila `discovery_result_wa` → wa_template_name
--    `diagnostico_es` con wa_components para el queue-sender (injectPayload):
--    cuerpo {{1}} = nombre · botón URL dinámica {{1}} = result_token
--    (la URL base https://discovery.appril.co/?r={{1}} vive en Meta).
-- 2. submit_discovery_lead v5: la rama de followup inmediato (flag
--    discovery_whatsapp_immediate_followup_enabled, HOY APAGADO) deja de
--    enviar el follow-up comercial por segmento y pasa a enviar EL RESULTADO
--    (discovery_result_wa) con payload {nombre, result_token} — la promesa
--    "le enviamos su informe por WhatsApp" se vuelve verdadera al encender.
--    Los follow-ups comerciales por segmento siguen existiendo como
--    templates/secuencias; solo dejan de colgar de este flag.
--
-- NADA se envía con esta migración: el flag sigue en false.

INSERT INTO public.message_templates
  (workspace_id, template_key, name, description, channel,
   wa_template_name, wa_language, wa_components, variables, status)
VALUES (
  'e2096477-fa6a-4b8f-a8b3-bd46ad720167',
  'discovery_result_wa',
  'Discovery · Resultado por WA (link)',
  'Envía el link permanente al resultado del Discovery (?r=token). Meta: diagnostico_es (Marketing, aprobado 30-jul-2026). Botón URL dinámica + 2 quick replies que atiende el agente comercial.',
  'whatsapp',
  'diagnostico_es',
  'es',
  '[
     {"type":"body","parameters":[{"type":"text","text":"{{nombre}}"}]},
     {"type":"button","sub_type":"url","index":"0","parameters":[{"type":"text","text":"{{result_token}}"}]}
   ]'::jsonb,
  '["nombre","result_token"]'::jsonb,
  'active'
)
ON CONFLICT (workspace_id, template_key) DO UPDATE SET
  ...
  wa_components = EXCLUDED.wa_components,
  variables = EXCLUDED.variables,
  status = EXCLUDED.status,
  updated_at = now();

-- 2) submit_discovery_lead v5 — idéntico a v4 salvo la rama IF v_immediate:
--    template fijo discovery_result_wa + payload {nombre, result_token}.
...
DECLARE
  ...
  v_result_base   CONSTANT text := 'https://discovery.appril.co/?r=';
...
BEGIN
  IF v_dl IS NOT NULL THEN
    ...
    IF v_dl_lead.id IS NOT NULL THEN
      IF v_whatsapp IS NOT NULL
         ...
         AND v_dl_lead.phone ~ '^\+[1-9][0-9]{7,14}$'
  ...
  IF v_lead_id IS NULL THEN
    INSERT INTO public.leads_master (
      ...
      v_workspace_id, v_full_name, v_whatsapp, v_email,
      nullif(trim(input->>'specialty'), ''), v_city, v_country,
      v_segment, (v_score_result->>'score')::integer, v_consent, v_email IS NOT NULL,
      'new', 'discovery_form', COALESCE(input->>'utm_campaign', 'discovery_form'), v_consent
    )
  ...
  INSERT INTO public.discovery_leads (
    workspace_id, lead_id, full_name, whatsapp_e164, email,
    city, country, clinic_name, consent, preferred_contact_channel,
    q_volume, q_lost, q_intent, q_urgency, q_ticket, q_digital,
    monthly_appointments_range, scheduling_method, average_ticket_range, lost_appointments_range,
    urgency, desired_next_step, raw_answers, score, marketing_segment, score_breakdown,
    lead_classification, calculation_version, agenda_maturity_level, annual_lost_revenue, hidden_cost_total,
    findings, frontend_calculations, completion_rate, started_at,
    utm_source, utm_medium, utm_campaign, utm_content, utm_term, landing_url,
    legacy_lead_score, scoring_version, score_input_debug, risk_dominant, risk_evidence,
    recommended_action, main_pain, main_pains, selected_currency, primary_cta_key, primary_cta_label
  ) VALUES (
    v_workspace_id, v_lead_id, v_full_name, v_whatsapp, v_email,
    v_city, v_country, v_clinic, v_consent, 'whatsapp',
    v_q_volume, v_q_lost, v_q_intent, v_q_urgency, v_q_ticket, v_q_digital,
    v_q_volume, v_q_digital, v_q_ticket, v_q_lost,
    v_q_urgency, v_q_intent,
    COALESCE(input->'raw_answers', input->'raw_payload', '{}'::jsonb),
    (v_score_result->>'score')::integer, v_segment, v_score_result->'breakdown',
    v_score_result->>'lead_classification', 'v2',
    v_maturity, v_annual_lost, v_hidden_cost,
    v_findings, v_fe, (input->>'completion_rate')::numeric, (input->>'started_at')::timestamptz,
    ...
    (v_score_result->>'score')::integer, v_scoring_version, v_score_input_dbg,
    v_findings->>'risk_dominant', v_findings->'risk_evidence',
    v_score_result->>'recommended_action', input->>'main_pain', input->'main_pains',
    v_fe->'currency'->>'selected_currency', input->>'primary_cta_key', input->>'primary_cta_label'
  )
  RETURNING id, result_token INTO v_discovery_id, v_result_token;

  v_result_url := v_result_base || v_result_token;
  ...
  VALUES (v_workspace_id, v_lead_id, 'discovery_form_submitted', 'funnel', jsonb_strip_nulls(v_event_meta));
  ...
  IF v_immediate THEN
    -- v5: se envía EL RESULTADO (link permanente), no el follow-up comercial.
    v_template_key := 'discovery_result_wa';

    INSERT INTO public.message_queue (
      workspace_id, lead_id, template_key, channel, to_address, payload, status, scheduled_at
    ) VALUES (
      v_workspace_id, v_lead_id, v_template_key, 'whatsapp',
      v_whatsapp,
      jsonb_build_object(
        'nombre',       split_part(v_full_name, ' ', 1),
        'result_token', v_result_token
      ),
      'pending', now()
    )
    ...
    INSERT INTO public.lead_events (workspace_id, lead_id, event_type, event_channel, metadata)
    VALUES (v_workspace_id, v_lead_id, 'message_queued', 'whatsapp',
      jsonb_build_object('message_queue_id', v_queue_id, 'template_key', v_template_key,
                         'discovery_lead_id', v_discovery_id, 'trigger', 'immediate'));
  ELSE
    v_template_key := NULL;
    v_queue_id     := NULL;
    INSERT INTO public.lead_events (workspace_id, lead_id, event_type, event_channel, metadata)
    VALUES (v_workspace_id, v_lead_id, 'followup_skipped', 'whatsapp',
      jsonb_build_object('stage', 'immediate', 'reason', 'flag_disabled',
                         'discovery_lead_id', v_discovery_id, 'segment', v_segment));
  END IF;
